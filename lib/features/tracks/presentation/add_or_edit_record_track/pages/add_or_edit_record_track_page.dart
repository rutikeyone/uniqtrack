...
import 'package:uniqtrack/features/tracks/presentation/add_or_edit_record_track/stores/states/states.dart';
...
class AddOrEditRecordTrackPage extends StatelessWidget {
  ...
  void _handleAddOrEditRecordTrackActions({
    required Activity<AddOrEditRecordTrackActions>? activity,
    ...
    action.when(
      navigateBack: () => _navigateBack(context),
    );
  }

  void _navigateBack(BuildContext context) {
    final navCallbackStore = context.read<AddOrEditRecordTrackNavCallbackStore>();
    navCallbackStore.navigateBack();
  }
      ...
      body: SafeArea(
        child: ReactionBuilder(
          builder: (context) {
            return reaction(
              (_) => store.actions,
              (activity) => _handleAddOrEditRecordTrackActions(
                activity: activity,
                context: context,
              ),
            );
